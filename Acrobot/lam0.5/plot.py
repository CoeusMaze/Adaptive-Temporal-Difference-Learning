...
for i in range(len(optimizers)):
    ...
    x = np.linspace(0, len(y)-1, num = len(y), dtype = int)
    plt.plot(x, y_mean)
    plt.fill_between(x, y_mean+y_std, y_mean-y_std, alpha=0.4, antialiased=True)
plt.legend(('AdaTD(0.5)','ALRR-TD(0.5)','TD(0.5)'))
